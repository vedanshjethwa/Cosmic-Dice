...
function App() {
  ...
  const generateWinningCards = (risk: RiskLevel) => {
    ...
    const targetCount = Math.max(
      1,
      Math.round(RISK_CONFIGS[risk].winningCardCount * winRate)
    );
  ...
  const handleCardClick = (index: number) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        const profit = isWinning ? amount * multiplier : 0;
        ...
        setStats(prev => ({
          totalProfit: prev.totalProfit + (isWinning ? profit - amount : -amount),
        ...
        const newPlay: GamePlay = {
          ...
          profit: isWinning ? profit - amount : -amount,
          timestamp: Date.now(),
          cardNumber: index + 1,
  ...
  const handleStarActivate = (newWinRate: number) => {
              ...
              <div className="bg-gradient-to-br from-blue-900/30 to-purple-900/20 rounded-xl p-6 panel-border mb-6 backdrop-blur-sm">
                ...
                <form onSubmit={handleBetSubmit}>
                  ...
                  {!isPlaying ? (
                    <button
                      type="submit"
                      disabled={!betAmount || Number(betAmount) <= 0 || Number(betAmount) > wallet}
